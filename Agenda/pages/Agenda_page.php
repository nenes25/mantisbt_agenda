...
?>
    ...
    <link rel='stylesheet' type='text/css'
          href='<?php echo plugin_file('bower/fullcalendar/dist/fullcalendar.css'); ?>'/>
    <link rel='stylesheet' type='text/css'
          href='<?php echo plugin_file('bower/fullcalendar/dist/fullcalendar.print.css'); ?>' media='print'/>
    <script type='text/javascript'
            src='<?php echo plugin_file('bower/fullcalendar/dist/fullcalendar.min.js'); ?>'></script>
<?php
...
?>
    <script type='text/javascript'
            src='<?php echo plugin_file('bower/fullcalendar/dist/lang/' . $codeLang . '.js'); ?>'></script>
    ...
    <input type="hidden" name="t_user_access_level" id="t_user_access_level"
           value="<?php echo $t_user_access_level; ?>"/>

    <?php #Gestion des traductions js?>
    <input type="hidden" id="mantis_agenda_translate_actionStart"
           value="<?php echo plugin_lang_get('action_start'); ?>"/>
    ...
    <input type="hidden" id="mantis_agenda_translate_noteDetails"
           value="<?php echo plugin_lang_get('note_details'); ?>"/>
    <input type="hidden" id="mantis_agenda_translate_author" value="<?php echo plugin_lang_get('author'); ?>"/>
    <input type="hidden" id="mantis_agenda_translate_seeBugDescription"
           value="<?php echo plugin_lang_get('see_bug_description'); ?>"/>
    ...
    <?php if (
        ...
        || ($g_due_date_update_threshold < DEVELOPER && $g_due_date_update_threshold > ADMINISTRATOR)
        || ($g_due_date_view_threshold < REPORTER && $g_due_date_view_threshold > REPORTER)
    ): ?>
        <div class="alert alert-warning center">
            <h4 class="alert-heading"><?php echo plugin_lang_get('configuration_error'); ?></h4>
            <ul>
                <li>time_tracking_enabled = ON</li>
                <li>due_date_update_threshold > = DEVELOPER</li>
                <li>due_date_view_threshold >= REPORTER</li>
            </ul>
        </div>
    <?php endif; ?>
    ...
    <div id="event_details" class="alert alert-info" role="alert">
        <div class="pull-right"><a id="event_details_close" href="#event_details"
                                   name="event_details">&times;</a>
        </div>
